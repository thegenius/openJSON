...
public:
	...
	Object& operator= (const Object& obj); /* be careful, there may exists memory leaking */
...
/************* begin of member function of class **************/
	...
	Object::~Object() {
		...
		if (this->ref_count <= 0) {
			...
			switch(this->type) {
				...
				default :		printf("%lx", (uint64_t)this);break;
				case CHAR_PTR:  printf("%lx", (uint64_t)this);
								delete this->data.str;
								break;
								/* print the structure type */
				case VEC_PTR:	printf("%lx:", (uint64_t)this);printf("[\n");
	...
	//	  					break;
	//	  	case VEC_PTR: 	len = this->data.vec->size();
	//						for (int i=0; i<len; ++i) {
	//							Object *tmp = &(*this)[i];
	//							printf("%llu",tmp);
	//							tmp->decrease_count();
	...
	//	  					//	char *str_tmp = it->first;
	//	  					//	Object *obj_tmp = it->second;
	//	  					//	delete str_tmp;
	//	  				//		delete obj_tmp;
	//	  				//	}
	//	  				//	delete this->data.map;
	...
	/***** begin of ref count manipulation*******/
	bool Object::count_no_positive() {
		return this->ref_count <= 0;
	}
	void Object::increase_count() {
		++this->ref_count;
	};
    void Object::decrease_count() {
		--this->ref_count;
	};
    /* end of ref count manipulation*/
	...
	Object& Object::operator[](const int idx) {
		...
		(*this->data.vec)[idx]->increase_count();
	...
	Object& Object::operator[](const char *key) {
		...
		if (this->type != MAP_PTR) {
			this->data.map = new unordered_map<char*,Object*>;
			(*this->data.map)[__key] = new Object();
			this->type = MAP_PTR;
		} else {
			if (this->data.map->find(__key) == this->data.map->end()) { //not exist
				(*this->data.map)[__key] = new Object();
			}
		}
		return *(*this->data.map)[__key];
	}
...
char* dump(const Object &obj) {
	...
	char *buffer = (char*)calloc(128,sizeof(char));
		...
		case UINT64:	sprintf(buffer,"%lu",obj.get_uint64()); return buffer;
		...
		case INT64:		sprintf(buffer,"%ld",obj.get_int64()); return buffer;
